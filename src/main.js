...
import { initFilters } from './js/modules/filters'
import { initSearch } from './js/modules/search'
import { subscribe, getFilteredCourses } from './js/modules/store'
...
function renderList(coursesArray) {
  const list = document.querySelector('.courses__list')
  list.innerHTML = coursesArray.map(createCard).join('')
}

const unsubscribe = subscribe((filtered) => {
  renderList(filtered)
})
